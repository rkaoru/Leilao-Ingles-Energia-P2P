...
contract MercadoEnergiaLeilao {
    ...
    function fazerProposta(uint _ofertaIndex, uint _quantidadeDesejada, uint _precoOferecidoPorKwhEmReais) public {
        ...
        require(_precoOferecidoPorKwhEmReais >= oferta.precoMinimoPorKwhEmReais, "Preco oferecido abaixo do minimo");

        uint precoTotalEmReais = _precoOferecidoPorKwhEmReais * _quantidadeDesejada;

        Proposta storage melhorProposta = melhoresPropostas[_ofertaIndex];
        if (melhorProposta.comprador == address(0) || precoTotalEmReais > (melhorProposta.precoOferecidoPorKwhEmReais * melhorProposta.quantidade)) {
